...
```{r}
ggplot(article.counts) + geom_histogram(aes(x = count), binwidth = 20)+ ggtitle("Histogram of Articles Written by Author")
```
...
```{r warning= FALSE}
ggplot(data = articles.counts.mwc, aes(x=count, y = mwc))+geom_point(aes(color=author), show.legend = FALSE) +geom_smooth(method = "lm", aes(color="red"), show.legend = FALSE) + ggtitle("Mean Wordcount by Total Articles")
